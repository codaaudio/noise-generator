...
def filter_signal_fir(signal_data, center_frequencies, fraction, fs, filter_length):
    ...
    print(
        f"Using FFT length: {fft_len} (original signal length: {len(signal_data)}, filter length: {filter_length})"
    )
...
def plot_crest_factors(
    ...
    plt.xticks(
        x_positions[::step],
        [
            f"{fc:.0f}" if fc >= 100 else f"{fc:.1f}"
            for fc in center_frequencies[::step]
        ],
        rotation=45,
        ha="right",
    )
    ...
    if final_signal is not None:
        ...
        ax2.semilogx(
            freqs[1:], np.unwrap(final_phases[1:]), "r-", alpha=0.6, label="Phase"
        )
    ...
    if filters is not None:
        ...
        ax1.set_title(
            f"Frequency Response of {filter_type.upper()} Filters\n1/{fraction} Octave Bands",
            fontsize=14,
            pad=20,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["iir", "fir"],
        default="fir",
        help="Filter type (default: fir)",
    )
    parser.add_argument(
        "--plot", "-p", action="store_true", help="Display bar plot of crest factors"
    )
    ...
    print(
        f"Filtering signal in {len(center_frequencies)} bands using {'FIR' if args.filter_type == 'fir' else 'IIR'} filters... ",
        end="",
        flush=True,
    )
    if args.filter_type == "fir":
        # Use FIR filters
        filtered_signals, filters = filter_signal_fir(
            signal_data, center_frequencies, args.fraction, fs, 65536
        )
    else:
        # Use IIR filters
        filtered_signals, filters = filter_signal_iir(
            signal_data, center_frequencies, args.fraction, fs
        )
    print("Done!")
